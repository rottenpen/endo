...
const loadWithoutErrorAnnotation = (
  ...
  const { importNowHook, moduleMap, moduleMapHook, moduleRecords } = weakmapGet(
    compartmentPrivateFields,
    compartment,
  );
  ...
  if (typeof aliasNamespace === 'string') {
    ...
    // Behold: recursion.
    // eslint-disable-next-line no-use-before-define
    const aliasRecord = memoizedLoadWithErrorAnnotation(
      compartmentPrivateFields,
      moduleAliases,
      alias.compartment,
      alias.specifier,
  ...
  const staticModuleRecord = importNowHook(moduleSpecifier);

  if (staticModuleRecord === null || typeof staticModuleRecord !== 'object') {
    Fail`importNowHook must return an object, for module ${q(
      moduleSpecifier,
...
/*
 * `loadSnc` synchronously gathers the `StaticModuleRecord`s for a module and its
...
export const loadSync = (
  ...
  /** @type {Map<object, Map<string, Record<any, any>>>} */

...
Options:
  ...
  --module, -m
    Evaluate code as a module which can therefore include \`import\`s.
  ...
  --arg NAME[~MAX|[+SEP]:VALUES], -a NAME[~MAX|[+SEP]:VALUES]
    ...
    either SEP or (if SEP is not present) any combination of spaces, tabs,
    and/or line feeds.
...
const mustReplace = (src, pairs) => {
  ...
  for (const [pattern, replacer] of pairs) {
    ...
    if (n > 0) continue;
    throw Error(`no match when replacing ${pattern}`);
  }
  return ret;
};
...
const { makeSimpleError, raiseError } = (() => {
  ...
  return {
    ...
    raiseError: (err, exitCode = 1) => {
      console.error(simpleErrors.has(err) ? err.message : err);
...
const main = async argv => {
  ...
  child.on('error', err => {
    if (err.code === 'ENOENT') {
      raiseError(`Error: missing required dependency ${cmd}`, EX_NOT_FOUND);
    } else if (err.code === 'EACCES') {
      raiseError(`Error: could not spawn ${cmd}`, EX_NOT_EXECUTABLE);
    } else {
      raiseError(err);
    }
    doneKit.reject(err);
